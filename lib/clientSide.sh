...
# Modify the base configuration file to reflet the client's need
# Most needs to match the server-side coniguration
# i.e. Fedora client needs the 'nobody' group, since the 'nogroup' does not exists
client_config() {
...
# Appends the certificates and keys within the 'client1.ovpn' file
generate_ovpn() {
